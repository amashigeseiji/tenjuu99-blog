...
// @test: tests/ssg-core/pageData.test.js
/**
 * 記事の文字列から frontmatter 部を解析し、フィールドの値を取り出す
 * @param {string} content - frontmatter を含む記事の文字列
 * @returns {Object<string, unknown>} frontmatter のフィールド値（frontmatter がなければ空オブジェクト）
 */
export const parseFrontmatter = (content) => {
  const matched = content.match(FRONTMATTER_REGEXP)
  return matched ? parseMetaData(matched.groups.variables) : {}
}
