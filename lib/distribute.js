...
import { createConverter } from '../packages/editor/server/image_upload.js'
...
const distribute = async (data, srcDir, distDir) => {
  ...
  for (const copyDir of distributeRaw) {
    ...
    if (config.image_converter) {
      const imageConverter = await createConverter(config.image_converter)
