 ...
 * @param {import('./publicationMeansResolver.js').ResolveOptions} options - 構成（blog.json の publish を展開したもの＋ cwd）
 * @returns {Promise<import('./publicationMeans.js').RemoteState>}
 */
export async function resolveRemoteState(options) {
  try {
    const resolved = await resolvePublicationMeans(options)
