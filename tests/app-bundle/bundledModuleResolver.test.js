...
describe('ネイティブアプリシェルは 実体の node_modules を持つ開発中プロジェクトを、既存物に書き込まずにコンテンツルートとして使える（同居できる）', () => {
  ...
  describe('同梱モジュール解決器は 同梱コードが使う依存パッケージへの参照を、コンテンツルートの中身に関わらず同梱コード自身の実体へ解決できる', () => {
    describe('同梱モジュール解決器は アプリ自身のパッケージ名への参照を同梱コードへ解決できる', () => {
      ...
      test('同梱ルートに解決できない不正なサブパスは通常の解決に委ねられる', () => {
        assert.equal(redirect('@tenjuu99/blog/', bundledAppRoot), null, '空サブパスは委ねる')
        assert.equal(redirect('@tenjuu99/blog/../escape.js', bundledAppRoot), null, '".." で同梱ルートの外へ出る指定子は委ねる')
        assert.equal(redirect('@tenjuu99/blog/lib/../../escape.js', bundledAppRoot), null, '途中の ".." で外へ出る指定子も委ねる')
        assert.equal(redirect('@tenjuu99/blog/%2e%2e/escape.js', bundledAppRoot), null, 'エンコードされた ".." も委ねる')
        assert.equal(redirect('@tenjuu99/blog//etc/escape.js', bundledAppRoot), null, '絶対パス風のサブパスは委ねる')
        assert.equal(
          redirect('@tenjuu99/blog/lib/../index.js', bundledAppRoot),
          'file:///bundle/Contents/Resources/app/index.js',
          '同梱ルート内に収まる ".." は許容される'
        )
      })

      test('自己参照でも依存パッケージでもない参照は通常の解決に委ねられる', () => {
        assert.equal(redirect('some-unbundled-package', bundledAppRoot), null, '他パッケージは redirect() の対象外')
        ...
        let delegated = null
        const outcome = resolve('some-unbundled-package', { parentURL: 'file:///x.js' }, (specifier, context) => {
          delegated = specifier
          return { url: 'file:///resolved-by-default.js' }
        })
        assert.equal(delegated, 'some-unbundled-package', 'フックは nextResolve に委ねる')
        assert.equal(outcome.url, 'file:///resolved-by-default.js')
      })
    })

    describe('同梱モジュール解決器は 依存パッケージ名判定器が一致と判定した参照を同梱コード自身の実体へ解決させられる', () => {
      test('依存パッケージへの参照は、同梱コード自身を起点に解決するよう nextResolve に委譲される', () => {
        let receivedContext = null
        const outcome = resolve(
          'sharp',
          { parentURL: 'file:///content-root/.cache/server/imageMetadataReader.js' },
          (specifier, context) => {
            receivedContext = context
            return { url: 'file:///resolved-by-node.js' }
          }
        )
        assert.equal(
          receivedContext.parentURL,
          pathToFileURL(`${repoRoot}/`).href,
          '同梱コード自身（アプリのルート）を起点に解決させる'
        )
        assert.equal(outcome.url, 'file:///resolved-by-node.js')
      })

      test('依存パッケージのサブパスへの参照も同様に委譲される', () => {
        let receivedContext = null
        resolve(
          'marked/lib/marked.esm.js',
          { parentURL: 'file:///content-root/.cache/server/x.js' },
          (specifier, context) => {
            receivedContext = context
            return { url: 'file:///resolved.js' }
          }
        )
        assert.equal(receivedContext.parentURL, pathToFileURL(`${repoRoot}/`).href)
      })

      test('（統合）コンテンツルート配下に複製されたコードから sharp を import しても解決できる', async () => {
        const contentRoot = await realpath(await mkdtemp(path.join(tmpdir(), 'copied-server-code-')))
        try {
          // packages/editor/server/* がコンテンツルートの .cache/server へ複製される状態を模す
          const copiedFile = path.join(contentRoot, '.cache/server/check-sharp.js')
          await mkdir(path.dirname(copiedFile), { recursive: true })
          await writeFile(copiedFile, 'import sharp from "sharp"\nconsole.log(typeof sharp)\n')

          const { stdout } = await execFileAsync(
            process.execPath,
            ['--import', registerScript, copiedFile],
            { cwd: contentRoot }
          )
          assert.equal(stdout.trim(), 'function', 'sharp モジュールが解決され読み込める')
        } finally {
          await rm(contentRoot, { recursive: true, force: true })
        }
      })
