...
describe('公開手段は公開済みの参照と公開物の反映を担い、手段を替えても公開を成立させられる', () => {
  it('git ではないインメモリの公開手段でも公開フロー（判定→反映→公開済み）が成立する', async () => {
    ...
    assert.strictEqual(await getPublicationStatus('src/pages/post/hello.md', means.remoteState), 'published')
  })
  ...
  describe('git公開手段は原稿を公開物としてリモートへの反映を実現できる', () => {
    it('公開物として原稿を申告し、リモートの参照と反映の実行を提供する', async () => {
      ...
      assert.strictEqual(typeof means.remoteState.existsInRemote, 'function')
      assert.strictEqual(typeof means.remoteState.diffFromRemote, 'function')
  ...
  describe('リモート状態解決器は公開手段が解決できなくても、参照不能な状態として提供を継続し、失敗をログに残せる', () => {
    it('手段が解決できるときは、そのリモート状態をそのまま返す', async () => {
      const { resolveRemoteState } = await import('../../lib/publishing/remoteStateResolver.js')
      const remoteState = await resolveRemoteState({ means: 'git', cwd: process.cwd() })
      assert.strictEqual(typeof remoteState.existsInRemote, 'function')
      assert.strictEqual(typeof remoteState.diffFromRemote, 'function')
    })
    ...
    it('差し替えた参照不能な状態は、公開ステータス判定器を通すと unknown になる', async () => {
      ...
      const { getPublicationStatus } = await import('../../packages/editor/server/publicationStatus.js')
      const remoteState = await resolveRemoteState({ means: 'ftp', cwd: process.cwd() })
      const status = await getPublicationStatus('src/pages/post/hello.md', remoteState)
