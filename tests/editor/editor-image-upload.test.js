...
describe('画像自動変換器は image_converter 設定に従い手動変換なしで画像を自動変換できる', () => {
  it('"sharp" 設定で変換器が解決されアップロードとビルドの両方で変換が動作する', async () => {
    // アップロード側: sharp ビルトインが解決される
    const { fn, ext } = await createConverter('sharp')
    assert.strictEqual(ext, 'webp')
    assert.strictEqual(typeof fn, 'function')

    // ビルド側: distributeImages が変換してdistに書き込む
    const tmpSrc = fs.mkdtempSync(path.join(os.tmpdir(), 'dist-root-src-'))
    const tmpDist = fs.mkdtempSync(path.join(os.tmpdir(), 'dist-root-dist-'))
    fs.writeFileSync(path.join(tmpSrc, 'photo.jpg'), Buffer.from('fake-image'))
    await distributeImages(tmpSrc, tmpDist, { fn: async (buf) => Buffer.from('converted'), ext: 'webp' })
...
describe('コンバーターファクトリー は ビルトイン名からビルトインコンバーターを解決できる', () => {
  it('"sharp" を指定したとき変換関数と出力拡張子 "webp" が得られる', async () => {
    const { fn, ext } = await createConverter('sharp')
    assert.strictEqual(ext, 'webp')
    assert.strictEqual(typeof fn, 'function')
  })
  ...
  describe('コンバーターファクトリー は "sharp" 指定でビルトインsharpコンバーターを読み込める', () => {
    it('"sharp" を指定したとき ext が "webp" になる（パススルーでなく sharp が解決された証拠）', async () => {
      const { ext } = await createConverter('sharp')
      assert.strictEqual(ext, 'webp')
    })
  })
...
// ─── 画像ライブラリからの画像追加（US-04） ───────────────────────────────────────────────

// @vocab: 画像ライブラリ
describe('画像ライブラリ は 記事編集を経由せずに画像を追加できる', () => {
  it('配置パスを指定した追加で画像が src/image 配下に保存され、追加日時が記録され、保存パスが返る', async () => {
    // ルートテスト — ツリーが完成するまで green にしない
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'library-add-test-'))
    const ledgerPath = path.join(tmpDir, 'image-library.json')
    const imageData = Buffer.from('fake-image-data').toString('base64')
    const result = await handleImageUpload(
      { imageData, imageFilename: 'photos/2026/test.jpg' },
      { converterFn: (buf) => buf, outputExt: null, baseDir: tmpDir, ledgerPath }
    )
    assert.strictEqual(result.imagePath, 'image/photos/2026/test.jpg')
    assert.strictEqual(result.markdownUrl, '/image/photos/2026/test.jpg')
    assert.ok(fs.existsSync(path.join(tmpDir, 'image/photos/2026/test.jpg')))
    assert.ok(getAddedAt(ledgerPath, 'image/photos/2026/test.jpg'))
    fs.rmSync(tmpDir, { recursive: true })
  })

  describe('パスリゾルバー は 記事に紐づかない画像の保存先を、指定された配置パスから導出できる', () => {
    it('ディレクトリ階層を含む配置パスをそのまま image/ 配下に導出する', () => {
      const result = resolveLibraryImagePath('photos/2026/test.jpg')
      assert.deepStrictEqual(result, {
        saveSubPath: 'image/photos/2026/test.jpg',
        markdownUrl: '/image/photos/2026/test.jpg'
      })
    })
    it('ファイル名のみの配置パスは image/ 直下に導出する', () => {
      const result = resolveLibraryImagePath('test.jpg')
      assert.deepStrictEqual(result, {
        saveSubPath: 'image/test.jpg',
        markdownUrl: '/image/test.jpg'
      })
    })
    it('outputExt を指定したとき拡張子を差し替える', () => {
      const result = resolveLibraryImagePath('photos/test.jpg', 'webp')
      assert.deepStrictEqual(result, {
        saveSubPath: 'image/photos/test.webp',
        markdownUrl: '/image/photos/test.webp'
      })
    })
    it('配置パスが image/ の外を指す場合はエラーになる', () => {
      assert.throws(() => resolveLibraryImagePath('../secret.jpg'), /不正な配置パス/)
      assert.throws(() => resolveLibraryImagePath('photos/../../secret.jpg'), /不正な配置パス/)
      assert.throws(() => resolveLibraryImagePath('/etc/malice.jpg'), /不正な配置パス/)
    })
    it('冗長な区切りは正規化される', () => {
      const result = resolveLibraryImagePath('./photos//test.jpg')
      assert.deepStrictEqual(result, {
        saveSubPath: 'image/photos/test.jpg',
        markdownUrl: '/image/photos/test.jpg'
      })
    })
  })

  describe('アップロードエンドポイント は 記事に紐づかない画像の追加を受け付け、既存の画像を上書きせずに保存して追加日時を記録できる', () => {
    it('既存の画像と同名の配置パスのとき保存せずエラーになり、既存の画像は変更されない', async () => {
      const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'library-add-test-'))
      const existingPath = path.join(tmpDir, 'image/photos/test.jpg')
      fs.mkdirSync(path.dirname(existingPath), { recursive: true })
      fs.writeFileSync(existingPath, 'existing-data')
      const imageData = Buffer.from('new-data').toString('base64')
      await assert.rejects(
        handleImageUpload(
          { imageData, imageFilename: 'photos/test.jpg' },
          { converterFn: (buf) => buf, outputExt: null, baseDir: tmpDir }
        ),
        /同名のファイルが既に存在します/
      )
      assert.strictEqual(fs.readFileSync(existingPath, 'utf-8'), 'existing-data')
      fs.rmSync(tmpDir, { recursive: true })
    })
    it('outputExt があるとき重複判定は変換後の保存パスに対して行われる', async () => {
      const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'library-add-test-'))
      const existingPath = path.join(tmpDir, 'image/photos/test.webp')
      fs.mkdirSync(path.dirname(existingPath), { recursive: true })
      fs.writeFileSync(existingPath, 'existing-data')
      const imageData = Buffer.from('new-data').toString('base64')
      await assert.rejects(
    ...
    it('記事に紐づく既存のアップロード（mdFile あり）の挙動は変わらない（上書き可）', async () => {
      const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'library-add-test-'))
      const imageData = Buffer.from('new-data').toString('base64')
      const first = await handleImageUpload(
        { imageData, imageFilename: 'photo.jpg', mdFile: 'book/test.md' },
        { converterFn: (buf) => buf, outputExt: null, baseDir: tmpDir }
      )
      const second = await handleImageUpload(
        { imageData, imageFilename: 'photo.jpg', mdFile: 'book/test.md' },
        { converterFn: (buf) => buf, outputExt: null, baseDir: tmpDir }
      )
      assert.strictEqual(first.markdownUrl, second.markdownUrl)
      fs.rmSync(tmpDir, { recursive: true })
    })
  })

  describe('新規作成UI は 画像ファイルを選んで配置パスを指定し、記事と同じ導線で画像を追加して表示対象にできる', () => {
    it('DOM配線のため自動テストなし（受け入れテスト・手動確認で検証）', () => {})
  })
})
