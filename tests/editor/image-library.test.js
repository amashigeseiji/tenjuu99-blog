...
describe('画像一覧コレクターは画像スキャナー・画像メタデータ読み取り器・画像台帳を組み合わせて画像一覧を作れる', () => {
  it('TODO: ルートテストでこの組み合わせを直接検証済み（重複回避のため個別テストは省略）', () => {})
})

// ─── 画像一覧表示（DOM描画・自動テストなし） ───────────────────────────────────────────────

describe('画像一覧表示は画像一覧コレクターの結果をサイドバーの「画像」タブにツリー表示できる', () => {
...
describe('画像ライブラリは画像の変化を一覧に反映し続け、記事編集画面と安全に共存できる ができる', () => {
  ...
  describe('画像一覧は画像タブを開くたびに最新の状態を反映できる', () => {
    it('tests/acceptance/editor-image-library.spec.ts「画像一覧は画像タブを開くたびに最新の状態を反映できる(F-01)」で検証済み', () => {})
  })
