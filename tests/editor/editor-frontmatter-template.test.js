...
describe('フロントマターテンプレートローダー は ディレクトリに対応するフロントマターテンプレートをエディタに挿入できる', () => {
  ...
  it('book/ ディレクトリの新規ファイルにはフロントマターテンプレートが挿入される', () => {
    const frontmatter = loadFrontmatterTemplate('book/my-book.md', templates)
  ...
  it('テンプレートがないディレクトリでは null を返す', () => {
    assert.strictEqual(loadFrontmatterTemplate('post/hello.md', templates), null)
  })

  it('ドットを含むファイル名でも title に正しいベース名が使われる', () => {
    const frontmatter = loadFrontmatterTemplate('book/my.note.v1.md', templates)
    assert.ok(frontmatter.includes('title: my.note.v1'))
  })
})
...
describe('テンプレートレゾルバー は テンプレート設定をサーバーから取得できる', () => {
  describe('ブログ設定から frontmatter_templates を読み取れる', () => {
    it('frontmatter_templates があれば返す', () => {
      const templates = [{ path_prefix: 'book/', fields: { title: '', price: '0' } }]
      assert.deepStrictEqual(getTemplates({ frontmatter_templates: templates }), templates)
    })
