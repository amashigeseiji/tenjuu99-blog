...
describe('サイドバーは各ファイルの公開ステータスを視覚的に表示できる', () => {
  ...
  describe('ファイルステータスコレクターはファイル群の公開ステータスをまとめて取得できる', () => {
    it('各ファイルマッピングの公開ステータスを一括取得して {treePath: status} マップを返す', async () => {
      ...
      const fileMappings = [
        { treePath: 'post/hello.md', localPath: 'src/pages/post/hello.md' },
        { treePath: 'post/world.md', localPath: 'src/pages/post/world.md' },
      ]
      const result = await collectStatuses(fileMappings, mockRemoteState)
