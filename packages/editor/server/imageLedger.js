 ...
 * @returns {Object.<string, { addedAt: string, publishedReferredBy?: string[], protected?: boolean, movedFrom?: string }>}
 */
 ...
 * 付け替えのたびに #移動元パス を記録する。すでに記録がある場合はそれを保つ
 * （a→b→c と移動を重ねても、リモートが知っているパス a を指し続ける）。
 * @param {string} ledgerPath
...
export function renameEntry(ledgerPath, oldImagePath, newImagePath) {
  ...
  ledger[newImagePath] = {
    ...ledger[oldImagePath],
    movedFrom: ledger[oldImagePath].movedFrom ?? oldImagePath,
  }
 ...
 * @vocab: 移動元パス
 * @test tests/editor/image-library.test.js
 * 記録がない画像パス・移動していない画像パスに対しては null を返す。
 * @param {string} ledgerPath
 * @param {string} imagePath
 * @returns {string|null}
 */
export function getMovedFrom(ledgerPath, imagePath) {
  return readLedger(ledgerPath)[imagePath]?.movedFrom ?? null
}

/**
 * @vocab: 移動元パス
 * @test tests/editor/image-library.test.js
 * #移動元パス の記録を消す。移動がリモートへ届いた時点で呼ばれる。
 * @param {string} ledgerPath
 * @param {string} imagePath
 * @returns {void}
 */
export function clearMovedFrom(ledgerPath, imagePath) {
  const ledger = readLedger(ledgerPath)
  if (!(imagePath in ledger) || !('movedFrom' in ledger[imagePath])) return
  delete ledger[imagePath].movedFrom
  writeFileSync(ledgerPath, JSON.stringify(ledger, null, 2))
}
