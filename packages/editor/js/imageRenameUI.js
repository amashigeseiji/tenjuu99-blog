 ...
 * 画像詳細表示（editor-options の右側）に改名操作を追加する。参照記事があれば
 * 参照記事一覧エンドポイントで確認したうえで確認ダイアログ（3択）を経て改名を実行する。
 * DOM描画・イベント配線に依存するため自動テストを持たない（手動確認のみ）。
 * @param {HTMLButtonElement} renameBtn
...
export function initImageRename(renameBtn, getEntry, getNewFileName, showConfirm, setFeedback, onRenamed) {
  ...
  renameBtn.addEventListener('click', async () => {
    ...
    } else if (!(await showConfirm(`「${entry.path}」を「${newFileName}」に改名します。よろしいですか？`))) {
      return
    }

    setFeedback('改名しています...')
    try {
      const res = await fetch('/rename_image', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ imagePath: entry.path, newFileName, referenceHandling })
      })
      const json = await res.json().catch(() => ({}))
      if (!json.success) {
        setFeedback(`改名できませんでした: ${json.error ?? '不明なエラー'}`)
        return
      }
      setFeedback('改名しました')
