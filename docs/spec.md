...
#### editor パッケージ — 公開手段の設定

エディターの公開・非公開・削除・取り込みは、`blog.json` の `publish` で選んだ**公開手段**を通してリモートに届きます。手段が替わっても操作の意味（公開ステータス・取り込みの安全性）は変わりません。

| 手段 | `publish.means` | 前提 | 説明 |
|------|-----------------|------|------|
| git（既定） | `"git"` または省略 | 手元に git がインストールされ、コンテンツルートが clone 済みで upstream が設定されている | commit / push / fetch で反映・取り込みを行う |
| GitHub | `"github"` | 手元に git は不要。GitHub がホストするリモートと、そこへ届く認証情報 | GitHub の API でリモートを直接読み書きする。git 運用のマシンと同じリモートを共有でき、反映は git の反映と見分けがつかない形（履歴が一本、内容が同じ）で行われる |

```json
{
  "publish": {
    "means": "github",
    "github": { "owner": "<所有者>", "repo": "<リポジトリ名>", "branch": "main" }
  }
}
```

- `github.branch` の既定は `main`。`github.api_base` で API の入口を変えられる（GitHub Enterprise 等。既定は `https://api.github.com`）
- **認証情報**は blog.json には書かない。コンテンツルート内の `.blog/credentials.json` に手段名で区切って置く（手元だけに残るファイル。git 運用のマシンでは `.gitignore` に `.blog/` を加える）:
  ```json
  { "github": { "token": "<GitHub のトークン（対象リポジトリの contents 読み書き権限）>" } }
  ```
  認証情報が無い・通らない・GitHub に届かないときは、公開ステータスが `unknown` になり、公開・取り込みは失敗として伝えられる
- **同期の基点**: GitHub 手段は「前回リモートと揃えた時点の内容」をファイルごとに `.blog/sync-base.json` に記録し、取り込みの安全判定（先行・後行・分岐・手元で消された・他所で作られた）に使う。`.cache/` と違い、消すと手元で編集しただけの記事が「両側で変わった」扱いに退化するので消さない
- **初回接触の基点揃え**: 基点の記録がまったく無いコンテンツルート（clone 済み相当のフォルダを初めて開いた）では、最初に手段を組み立てたときにリモートと同じ内容のファイルすべての基点をまとめて揃える。以降は差分判定で「同じ」と分かった時点でファイルごとに揃える。初回接触より前にリモートだけが進んでいたファイルは基点が無く、分岐扱いになる
