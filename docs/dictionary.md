...
**最終更新:** 2026-06-27（editor-publish 昇格 - 公開ステータス・公開する・更新する・公開済み状態・公開フィードバック・公開対象・非公開にする・削除する・デプロイ・公開ハンドラー・公開ステータス判定器・公開対象コレクター・画像参照抽出器・変更反映器を追加）
...
- `references`: [リアルタイムプレビュー更新](#リアルタイムプレビュー更新) — 挿入完了が即時更新の即時トリガーとなる
...
**関係:**
- `references`: [プレビュー](#プレビュー) — プレビューを自動更新する
- `references`: [テキスト入力](#テキスト入力) — テキスト入力の停止がトリガーとなる
- `references`: [インライン画像挿入](#インライン画像挿入) — 挿入完了が即時トリガーとなる
...
**関係:**
- `references`: [リアルタイムプレビュー更新](#リアルタイムプレビュー更新) — 入力停止が更新トリガーとなる
- `references`: [デバウンサー](#デバウンサー) — 「停止」を検知する装置
...
en: Publish
**定義:** [公開ステータス](#公開ステータス)の `未公開 → 公開済み` 遷移を起動するアクション。Publishボタンで実行される。[保存](#保存)とは別操作（保存はローカル書き込みのみ）。コミットメッセージの入力なしに完結する。
**関係:**
...
**関係:**
- `references`: [デバウンサー](#デバウンサー) — デバウンサーを使用する
- `references`: [ドロップレシーバー拡張](#ドロップレシーバー拡張) — 協調してプレビューの自動更新を実現する
- `references`: [リアルタイムプレビュー更新](#リアルタイムプレビュー更新) — を実現する
...
**関係:**
- `references`: [プレビュー自動更新器](#プレビュー自動更新器) — に組み込まれる
- `references`: [テキスト入力](#テキスト入力) — テキスト入力の停止を検知する
...
**定義:** 既存の[ドロップレシーバー](#ドロップレシーバー)に追加される、画像挿入完了後に更新コールバックを呼び出す拡張。[インライン画像挿入](#インライン画像挿入)の即時プレビュー更新を実現する。
**関係:**
- `references`: [ドロップレシーバー](#ドロップレシーバー) — ドロップレシーバーの拡張
- `references`: [リアルタイムプレビュー更新](#リアルタイムプレビュー更新) — 即時トリガー経路を担う
...
en: PublishHandler
**定義:** HTTPリクエストを受け取り、[公開対象コレクター](#公開対象コレクター)で対象を収集し、[変更反映器](#変更反映器)で[公開する](#公開する)または[更新する](#更新する)遷移を実行し、[公開フィードバック](#公開フィードバック)をレスポンスとして返す装置。ファイルの保存も一括して処理する。
**関係:**
